...
%% Read the baseline flows and save the 1990s winter flows in an Excel file
    ...
    %Add only the winter flows for the analysis
    for j=1:length(Check_months)
        Check_winter_months(j,1)=ismember(Check_months(j,1),Winter_months);
    end
    
    [delete_idx,~]=find(Check_winter_months==0);
    ...
    if exist(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Baseline\Winter\Baseline_station_',no,'.xls'), 'file')==2
        delete(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Baseline\Winter\Baseline_station_',no,'.xls'));
    end
    writetable(Current_station_baseline,strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Baseline\Winter\Baseline_station_',no,'.xls'));
     
    %% Read the future flows and save the 2030s future winter flows in an Excel file
    Check_months=[];
    Check_winter_months=[];
    ...
    Replace_Start_Date=datetime(2020,02,29);
    [replace_nat_idx,~]=find(isnat(Time_Sim_future_2030s));
    for k=1:length(replace_nat_idx)
        Months_to_replace(k,1)=Replace_Start_Date;
        if year(Replace_Start_Date)==2020|year(Replace_Start_Date)==2024|year(Replace_Start_Date)==2028|year(Replace_Start_Date)==2032|year(Replace_Start_Date)==2036|year(Replace_Start_Date)==2040|year(Replace_Start_Date)==2044|year(Replace_Start_Date)==2048            
            Replace_Start_Date=Replace_Start_Date + years(1);
        else            
            Replace_Start_Date=Replace_Start_Date;
            check=isequal(Months_to_replace(k-1,1),Months_to_replace(k,1));
            if check==1
                Replace_Start_Date=Replace_Start_Date + years(1);
            end
        end
    ...
    Time_Sim_future_2030s(replace_nat_idx,:)=Months_to_replace;
    ...
    %Check if file exists, if yes delete it
    if exist(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Future_2030s\Winter\Future_2030s_station_',no,'.xls'), 'file')==2
        delete(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Future_2030s\Winter\Future_flow_inputs\Future_2030s_station_',no,'.xls'));
    end
...
%% Read the future flows and save the 2050s future winter flows in an Excel file
   Check_months=[];
    Check_winter_months=[];
    ...
    Check_index=[1:length(Flow_Sim_future_2050s_RCM01)];
    
    for j=1:length(Flow_Sim_future_2050s_RCM01)
        Flow_Sim_future_2050s_allRCM(j,1)=mean(Flow_Sim_future_2050s(j,:));
    end

    Replace_Start_Date=datetime(2040,02,29);
    [replace_nat_idx,~]=find(isnat(Time_Sim_future_2050s));
    for k=1:length(replace_nat_idx)
        Months_to_replace(k,1)=Replace_Start_Date;
        if year(Replace_Start_Date)==2040|year(Replace_Start_Date)==2044|year(Replace_Start_Date)==2048|year(Replace_Start_Date)==2052|year(Replace_Start_Date)==2056|year(Replace_Start_Date)==2060|year(Replace_Start_Date)==2064|year(Replace_Start_Date)==2068            
            Replace_Start_Date=Replace_Start_Date + years(1);
        else            
            Replace_Start_Date=Replace_Start_Date;
            check=isequal(Months_to_replace(k-1,1),Months_to_replace(k,1));
    ...
    Time_Sim_future_2050s(replace_nat_idx,:)=Months_to_replace;
    ...
    %Check if file exists, if yes delete it
    if exist(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Future_2050s\Winter\Future_2050s_station_',no,'.xls'), 'file')==2
        delete(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Future_2050s\Winter\Future_2050s_station_',no,'.xls'));
    end
    writetable(Current_station_future_2050s,strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Future_2050s\Winter\Future_2050s_station_',no,'.xls'));
 
%% Plot the baseline winter flow vs 2030s winter flow for all RCMs
clf %clears figure information
       set(gca,'Box','on');
       hold on
       save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Baseline_vs_2030s_all_RCM_ID_',no,'.png');
       tiledlayout(2,1)
 
       %Top plot
       nexttile
       box on
       title_name_baseline=strcat('Daily observed baseline winter flows at station no.'," ",no);
       ...
       nexttile
       box on
       title_name_baseline=strcat('Daily observed future 2030s winter flows at station no.'," ",no," ",'for all RCMs');
       title(title_name_baseline);
       xlabel('Time');
       ylabel('Flow [m^3/s]');
       hold on
...
%% Plot the baseline winter flow vs 2050s winter flow
clf %clears figure information
       set(gca,'Box','on');
       save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Baseline_vs_2050s_all_RCM_ID_',no,'.png');
       tiledlayout(2,1)
       ...
       nexttile
       box on
       title_name_future_2030s=strcat('Daily observed future 2050s winter flows at station no.'," ",no," ",'for all RCMs');
...
%% Plot daily future 2030s winter flows for all RCMs
clf %clears figure information
       tiledlayout(4,3,'TileSpacing','Compact');
       set(gca,'Box','on','FontSize',9);
       hold on
       save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Daily_future_flows_2030s_RCM01_ID_',no,'.png');
       title_name_future=strcat('Daily future 2030s winter flows at station no.'," ",no);
...
%% Plot daily future 2050s winter flows for all RCMs
clf %clears figure information
       tiledlayout(4,3,'TileSpacing','Compact');
       set(gca,'Box','on','FontSize',9);
       hold on
       save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Daily_future_flows_2050s_RCM01_ID_',no,'.png');
       title_name_future=strcat('Daily future 2050s winter flows at station no.'," ",no);
       title(title_name_future);       
       hold on
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%% Get monthly baseline winter flows
Table_baseline=[];
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Get monthly future 2030s winter flows
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Get monthly future 2050s winter flows
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Table_monthly_header={'Baseline','Baseline monthly mean winter flows [m^3/s]','RCM01 monthly mean winter flow [m^3/s]','RCM04 monthly mean winter flow [m^3/s]','RCM05 monthly mean winter flow [m^3/s]','RCM06 monthly mean winter flow [m^3/s]','RCM07 monthly mean winter flow [m^3/s]','RCM08 monthly mean winter flow [m^3/s]','RCM09 monthly mean winter flow [m^3/s]','RCM10 monthly mean winter flow [m^3/s]','RCM11 monthly mean winter flow [m^3/s]','RCM12 monthly mean winter flow [m^3/s]','RCM13 monthly mean winter flow [m^3/s]','RCM15 monthly mean winter flow [m^3/s]','All RCM future monthly mean winter flow [m^3/s]','Baseline-RCM01 [%]','Baseline-RCM04 [%]','Baseline-RCM05 [%]','Baseline-RCM06 [%]','Baseline-RCM07 [%]','Baseline-RCM08 [%]','Baseline-RCM09 [%]','Baseline-RCM10 [%]' ,'Baseline-RCM11 [%]','Baseline-RCM12 [%]','Baseline-RCM13 [%]','Baseline-RCM15 [%]','Baseline mean-Future mean flow [%]'};
 
%Write the table into an excel file
 
if exist(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2030s_monthly_vs_baseline.xls'), 'file')==2
   delete(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2030s_monthly_vs_baseline.xls'));
end
 
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2030s_monthly_vs_baseline.xls'),Table_monthly,'Sheet1','A2');
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2030s_monthly_vs_baseline.xls'),Table_monthly_header,'Sheet1','A1');
 
%% Plot the differences in monthly mean winter flows
...
%% Difference in monthly mean winter flows for RCM01
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM01_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM01');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for RCM04
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM04_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM04');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for RCM05
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM05_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM05');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for RCM06
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM06_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM06');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for RCM07
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM07_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM07');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for RCM08
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM08_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM08');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for RCM09
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM09_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM09');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for RCM10
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM10_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM10');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for RCM11
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM11_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM11');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for RCM12
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM12_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM12');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for RCM13
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM13_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM13');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for RCM15
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_RCM15_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM15');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Difference in monthly mean winter flows for all RCMs
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Monthly_mean_flows_diff_allRCM_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for all RCMs');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
Table_monthly_header={'Baseline','Baseline monthly mean winter flows [m^3/s]','Future','RCM01 monthly mean winter flow [m^3/s]','RCM04 monthly mean winter flow [m^3/s]','RCM05 monthly mean winter flow [m^3/s]','RCM06 monthly mean winter flow [m^3/s]','RCM07 monthly mean winter flow [m^3/s]','RCM08 monthly mean winter flow [m^3/s]','RCM09 monthly mean winter flow [m^3/s]','RCM10 monthly mean winter flow [m^3/s]','RCM11 monthly mean winter flow [m^3/s]','RCM12 monthly mean winter flow [m^3/s]','RCM13 monthly mean winter flow [m^3/s]','RCM15 monthly mean winter flow [m^3/s]','All RCM future monthly mean winter flow [m^3/s]','Baseline-RCM01 [%]','Baseline-RCM04 [%]','Baseline-RCM05 [%]','Baseline-RCM06 [%]','Baseline-RCM07 [%]','Baseline-RCM08 [%]','Baseline-RCM09 [%]','Baseline-RCM10 [%]' ,'Baseline-RCM11 [%]','Baseline-RCM12 [%]','Baseline-RCM13 [%]','Baseline-RCM15 [%]','Baseline mean-Future mean flow [%]'};
 
%Write the table into an excel file
 
if exist(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2050s_monthly_vs_baseline.xls'), 'file')==2
   delete(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2050s_monthly_vs_baseline.xls'));
end
 
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2050s_monthly_vs_baseline.xls'),Table_monthly,'Sheet1','A2');
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2050s_monthly_vs_baseline.xls'),Table_monthly_header,'Sheet1','A1');
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM01_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM01');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM04_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM04');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM05_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM05');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM06_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM06');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM07_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM07');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM08_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM08');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM09_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM09');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM10_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM10');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM11_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM11');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM12_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM12');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM13_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM13');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_RCM15_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM15');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Monthly_mean_flows_diff_allRCM_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s monthly mean winter flows and baseline winter flows at station no.'," ",no," ",'for all RCMs');
title(title_name_baseline);
xlabel('Months');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Get annual baseline winter flows
Table_baseline=[];
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Get annual future 2030s mean winter flows
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
%% Calculate the mean annual winter flow considering all the RCMs
%% All RCMs
...
%% Get annual future 2050s mean winter flows
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Table_annual_header={'Baseline','Baseline annual mean winter flows [m^3/s]','Future','RCM01 annual mean winter flow [m^3/s]','RCM04 annual mean winter flow [m^3/s]','RCM05 annual mean winter flow [m^3/s]','RCM06 annual mean winter flow [m^3/s]','RCM07 annual mean winter flow [m^3/s]','RCM08 annual mean winter flow [m^3/s]','RCM09 annual mean winter flow [m^3/s]','RCM10 annual mean winter flow [m^3/s]','RCM11 annual mean winter flow [m^3/s]','RCM12 annual mean winter flow [m^3/s]','RCM13 annual mean winter flow [m^3/s]','RCM15 annual mean winter flow [m^3/s]','All RCM future annual mean winter flow [m^3/s]','Baseline-RCM01 [%]','Baseline-RCM04 [%]','Baseline-RCM05 [%]','Baseline-RCM06 [%]','Baseline-RCM07 [%]','Baseline-RCM08 [%]','Baseline-RCM09 [%]','Baseline-RCM10 [%]' ,'Baseline-RCM11 [%]','Baseline-RCM12 [%]','Baseline-RCM13 [%]','Baseline-RCM15 [%]','Baseline-Future annual mean flow [%]'};
 
%Write the table into an excel file
 
if exist(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2030s_annual_vs_baseline.xls'), 'file')==2
   delete(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2030s_annual_vs_baseline.xls'));
end
 
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2030s_annual_vs_baseline.xls'),Table_annual,'Sheet1','A2');
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2030s_annual_vs_baseline.xls'),Table_annual_header,'Sheet1','A1');
 
%% Plot the differences in annual mean winter flows
...
%% Difference in annual mean winter flows for RCM01
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM01_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM01');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM01 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM01_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM01');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
hold on
hold on
legend('Baseline annual mean winter flow [m^3/s]','Future 2030s annual mean winter flow [m^3/s]','Location','northeast');
...
%% Difference in annual mean winter flows for RCM04
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM04_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM04');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM04 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM04_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM04');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
%% Difference in annual mean winter flows for RCM05
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM05_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM05');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM05 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM05_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM05');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
%% Difference in annual mean winter flows for RCM06
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM06_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM06');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM06 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM06_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM06');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
%% Difference in annual mean winter flows for RCM07
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM07_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM07');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM07 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM07_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM07');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
%% Difference in annual mean winter flows for RCM08
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM08_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM08');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM08 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM08_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM08');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
%% Difference in annual mean winter flows for RCM09
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM09_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM09');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM09 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM09_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM09');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
%% Difference in annual mean winter flows for RCM10
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM10_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM10');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM10 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM10_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM10');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
%% Difference in annual mean winter flows for RCM11
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM11_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM11');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM11 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM11_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM11');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
%% Difference in annual mean winter flows for RCM12
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM12_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM12');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM12 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM12_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM12');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
%% Difference in annual mean winter flows for RCM13
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM13_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM13');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM13 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM13_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM13');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
%% Difference in annual mean winter flows for RCM15
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_RCM15_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM15');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for RCM15 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_RCM15_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM15');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
%% Difference in annual mean winter flows for all RCMs
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Annual_mean_flows_diff_allRCM_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2030s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for all RCMs');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2030s annual mean winter flows vs baseline mean flows for all RCMs as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2030s\Future_2030s_allRCM_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2030s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for all RCMs');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
Table_annual_header={'Baseline','Baseline annual mean winter flows [m^3/s]','RCM01 annual mean winter flow [m^3/s]','RCM04 annual mean winter flow [m^3/s]','RCM05 annual mean winter flow [m^3/s]','RCM06 annual mean winter flow [m^3/s]','RCM07 annual mean winter flow [m^3/s]','RCM08 annual mean winter flow [m^3/s]','RCM09 annual mean winter flow [m^3/s]','RCM10 annual mean winter flow [m^3/s]','RCM11 annual mean winter flow [m^3/s]','RCM12 annual mean winter flow [m^3/s]','RCM13 annual mean winter flow [m^3/s]','RCM15 annual mean winter flow [m^3/s]','All RCM future annual mean winter flow [m^3/s]','Baseline-RCM01 [%]','Baseline-RCM04 [%]','Baseline-RCM05 [%]','Baseline-RCM06 [%]','Baseline-RCM07 [%]','Baseline-RCM08 [%]','Baseline-RCM09 [%]','Baseline-RCM10 [%]' ,'Baseline-RCM11 [%]','Baseline-RCM12 [%]','Baseline-RCM13 [%]','Baseline-RCM15 [%]','Baseline-Future annual mean winter flow [%]'};
%Write the table into an excel file
 
if exist(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2050s_annual_vs_baseline.xls'), 'file')==2
   delete(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2050s_annual_vs_baseline.xls'));
end
 
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2050s_annual_vs_baseline.xls'),Table_annual,'Sheet1','A2');
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_flows_2050s_annual_vs_baseline.xls'),Table_annual_header,'Sheet1','A1');
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM01_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM01');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM01 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM01_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM01');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
hold on
hold on
legend('Baseline annual mean winter flow [m^3/s]','Future 2050s annual mean winter flow [m^3/s]','Location','northeast');
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM04_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM04');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM04 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM04_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM04');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM05_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM05');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM05 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM05_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM05');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM06_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM06');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM06 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM06_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM06');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM07_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM07');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM07 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM07_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM07');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM08_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM08');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM08 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM08_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM08');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM09_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM09');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM09 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM09_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM09');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM10_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM10');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM10 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM10_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM10');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM11_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM11');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM11 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM11_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM11');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM12_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM12');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM12 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM12_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM12');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM13_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM13');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM13 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM13_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM13');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_RCM15_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for RCM15');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for RCM15 as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_RCM15_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for RCM15');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
clf %clears figure information
set(gca,'Box','on');       
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Annual_mean_flows_diff_allRCM_ID_',no,'.png');
title_name_baseline=strcat('Difference between future 2050s annual mean winter flows and baseline winter flows at station no.'," ",no," ",'for all RCMs');
title(title_name_baseline);
xlabel('Years');
ylabel('Difference [%]');
xlim([1 x_max]);
...
%% Plot the 2050s annual mean winter flows vs baseline mean flows for all RCMs as a bar graph
clf %clears figure information
set(gca,'Box','on');
save_name_baseline=strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Future_2050s\Future_2050s_allRCM_bar_chart_annual_mean_flows_ID_',no,'.png');
title_name_baseline=strcat('Future 2050s annual mean winter flows vs baseline annual mean winter flows at station no.'," ",no," ",'for all RCMs');
title(title_name_baseline);
xlabel('Years');
ylabel('Flows [m^3/s]');
hold on
...
Table_final_header={'Station ID','Baseline mean winter flow [m^3/s]','Future 2030s mean winter flow [m^3/s]','Future 2050s mean winter flow [m^3/s]'};
 
if exist(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_final_annual_vs_baseline_means.xls'), 'file')==2
   delete(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_final_annual_vs_baseline_means.xls'));
end
 
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_final_annual_vs_baseline_means.xls'),Table_final,'Sheet1','A2');
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_final_annual_vs_baseline_means.xls'),Table_final_header,'Sheet1','A1');
...
Table_final_header={'Station ID','Baseline total winter flows [m^3/s]','Future 2030s total winter flows [m^3/s]','Future 2050s total winter flows [m^3/s]'};
 
if exist(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_final_annual_vs_baseline_sum.xls'), 'file')==2
   delete(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_final_annual_vs_baseline_sum.xls'));
end
 
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_final_annual_vs_baseline_sum.xls'),Table_final,'Sheet1','A2');
xlswrite(strcat('H:\01.PhD\004.Chapter4\00.Results\Flows\All_stations\Winter\Station_',no,'_final_annual_vs_baseline_sum.xls'),Table_final_header,'Sheet1','A1');
